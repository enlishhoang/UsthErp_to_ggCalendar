...
if %errorlevel% neq 0 (
    echo [1/4] Khong tim thay Python. Dang tai Python 3.13.0 tu trang chu...
    curl -# -o python_installer.exe https://www.python.org/ftp/python/3.13.0/python-3.13.0-amd64.exe
    
    echo [2/4] Dang cai dat Python va tu dong them vao PATH...
    ...
    echo [1/4] Python da duoc cai dat san tren may.
    echo [2/4] Bo qua buoc cai dat Python.
)

echo.
echo [3/4] Dang tai va cai dat cac thu vien (Global)...
:: Su dung 'py -m' de luon goi dung ban Python vua cai ma khong bi vuong loi PATH
py -m pip install --upgrade pip
py -m pip install requests pycryptodome nest-asyncio playwright google-api-python-client google-auth-httplib2 google-auth-oauthlib beautifulsoup4

echo.
echo [4/4] Dang tai trinh duyet Chromium cho Playwright...
